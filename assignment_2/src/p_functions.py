...
def func(p,value):
    ...
    # print value
    if len(p) == 2:
        if p[1] in keywords: # Create node
            n += 1
            p[1] = [n,p[1]]
    ...
    for i in range(0,len(p)):
        # print p[i]
        if i == 0: # Non_terminal, create node
            n += 1
            p[i] = [n,value]
            print "\t", p[i][0], "[label=\""+p[i][1]+"\"]"
        else:
            if isinstance(p[i],list): # Non-terminal, create edge
                print "\t",p[i][0],'->',p[0][0]
                if i != len(p)-1:
                    s += str(p[i][0])+"->"
                else:
                    s += str(p[i][0])
            elif p[i] in keywords:
                key_no += 1
            else: # Terminal, create node + edge
                n += 1
                p[i] = [n,p[i]]
